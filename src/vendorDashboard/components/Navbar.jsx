  ...
  return (
    <Wrapper>
      <div className="navbar px-3 py-2 flex justify-between h-[80px] w-[80%] mx-auto items-center text-slate-900">
        ...
        <p className='cursor-pointer' onClick={navigat}>{vendorFirmName}</p>
        <div className="right">
          {!showLogout ? <button onClick={showDrawer}>
            <span className='flex items-center'><CgProfile className='mr-1' /><span>Login</span></span>
          </button>
          :
          <button className="ml-2" onClick={logoutHandler}>
          <span className='flex items-center'><CgLogOut className='mr-1' /><span>Logout</span></span>
          </button>
          }
        </div>
      </div>
